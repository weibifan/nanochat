"""
Unified evaluation script for base models (_zh 版：CORE 评测本地优先).

本文件是 scripts/base_eval.py 的 _zh 拷贝，仅改动 CORE 评测的数据获取：
优先使用本地 eval_data/eval_bundle.zip（nanochat/eval_data_zh.py 解压到
<base_dir>/eval_bundle），本地缺失时才回退到原版 S3 下载。其余逻辑与原版一致。
...
Examples:
    ...
    torchrun --nproc_per_node=8 -m scripts.base_eval_zh --model-tag d24 --device-batch-size=16

    # Quick/approximate evaluation using a single GPU
    python -m scripts.base_eval_zh --model-tag d24 --device-batch-size=16 --max-per-task=100 --split-tokens=524288
"""
import os
import csv
import time
...
def evaluate_core(model, tokenizer, device, max_per_task=-1):
    ...
    base_dir = get_base_dir()
    eval_bundle_dir = os.path.join(base_dir, "eval_bundle")
    # 优先使用本地 eval_data/eval_bundle.zip；本地缺失才回退 S3 下载
    if not os.path.exists(eval_bundle_dir):
        if not ensure_core_eval_bundle():
