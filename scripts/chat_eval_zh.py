"""
Evaluate the Chat model (_zh 版：ChatCORE 数据本地优先).

本文件是 scripts/chat_eval.py 的 _zh 拷贝，改动两处：
1. main() 启动时先 install_chatcore_datasets()：把本地 eval_data/ 下的
   ARC/MMLU/GSM8K/HumanEval parquet 安装进 <base_dir>/task_data/ 缓存，
   tasks/* 随即读本地，不再联网。
2. 评测用任务类固定取**原版**（MMLU 经 importlib 绕过可能被 install_zh_modules
   重定向到带过滤的 mmlu_zh；过滤会剔除超长测试题，影响评测准确性）。

All the generic code lives here, and all the evaluation-specific
code lives in nanochat directory and is imported from here.

Example runs:
python -m scripts.chat_eval_zh -i sft -a ARC-Easy
torchrun --nproc_per_node=8 -m scripts.chat_eval_zh -- -i sft -a ARC-Easy
"""
...
from nanochat.eval_data_zh import install_chatcore_datasets
...
def _load_original_task(module_name):
    """加载原版任务模块（绕过 sys.modules 里可能存在的 *_zh 重定向）。"""
    tasks_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "tasks")
    spec = importlib.util.spec_from_file_location(
        f"tasks.{module_name}_orig", os.path.join(tasks_dir, f"{module_name}.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


MMLU = _load_original_task("mmlu").MMLU
...
if __name__ == "__main__":
    ...
    # 本地 eval_data 优先：把 ARC/MMLU/GSM8K/HumanEval parquet 装进 task_data 缓存
    install_chatcore_datasets()
