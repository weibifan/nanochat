"""
Supervised fine-tuning (SFT) the model (_zh 版：ChatCORE 数据本地优先).

本文件是 scripts/chat_sft.py 的 _zh 拷贝，改动三处：
1. run_chat_eval 改从 scripts/chat_eval_zh 导入（其评测用原版任务类，
   MMLU 不会被 install_zh_modules 重定向成带过滤的 mmlu_zh）。
2. 启动时调用 install_chatcore_datasets()：把本地 eval_data/ 下的
   ARC/MMLU/GSM8K/HumanEval parquet 装进 <base_dir>/task_data/ 缓存，
   --chatcore-every 开启后 ChatCORE 评测即可离线运行。
3. 其余训练逻辑（含 mmlu_zh/smoltalk_zh 过滤训练数据）与原版一致。

Run as:

python -m scripts.chat_sft_zh

Or torchrun for training:

torchrun --standalone --nproc_per_node=8 -m scripts.chat_sft_zh -- --device-batch-size=16
"""
...
from scripts.chat_eval_zh import run_chat_eval
